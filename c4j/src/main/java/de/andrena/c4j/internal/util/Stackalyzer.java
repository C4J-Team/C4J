...
public class Stackalyzer {
	...
	public byte[] getDependenciesFor(CtBehavior contractBehavior, int indexOfDependentCall) throws BadBytecode,
			NotFoundException, RTFMException {
	...
	private void checkLoadOpcodes(CodeIterator ci, int numParams, int index, int op) throws RTFMException {
		switch (op) {
			case ALOAD_1:
			case DLOAD_1:
			case FLOAD_1:
	...
	private void verifyLocalVarAccess(int i, int numParams) throws RTFMException {
		if (i > numParams) {
			throw new RTFMException("Illegal access on local variable within old().");
		}
	}

	private int getOpcodeDelta(int op, int index, CodeIterator ci, ConstPool constPool)
			throws BadBytecode, NotFoundException, RTFMException {
		switch (op) {
			case GETFIELD:
			case GETSTATIC:
				// either +0 or +1
